...
void CONFIGURATION_parseFileContent(char * fileContent, ConfigParams * configParams) {
  ...
  configParams->telescopeName = malloc(strlen(fileParam) * sizeof(char));
  strcpy(configParams->telescopeName, fileParam);
  free(fileParam);
  fileParam = NULL;
  i++;
  ...
  // Temps
  CONFIGURATION_splitFileContent(fileContent, &fileParam, &i);
  configParams->waitTime = atoi(fileParam);
  free(fileParam);
  fileParam = NULL;
  i++;
  ...
  // Port
  CONFIGURATION_splitFileContent(fileContent, &fileParam, &i);
  configParams->port = atoi(fileParam);
  free(fileParam);
  fileParam = NULL;
  i++;
  ...
  // Directori
  CONFIGURATION_splitFileContent(fileContent, &fileParam, &i);
  configParams->filesDirectory = malloc(strlen(fileParam) * sizeof(char));
  strcpy(configParams->filesDirectory, fileParam);
  free(fileParam);
  fileParam = NULL;
}
